...
class TaskPanel(QWidget):
    ...
    duplicate_task_requested = pyqtSignal(int)
    ...
    def __init__(self) -> None:
        ...
        self.duplicate_button = QPushButton("复制任务")
    ...
    def set_tasks(self, tasks: list[Task]) -> None:
        ...
        for task in tasks:
            ...
            limit = "不限次数" if task.max_executions <= 0 else f"最多 {task.max_executions} 次"
            item = QListWidgetItem(f"{task.name}\n{task.interval_ms / 1000:g}s | {limit} | {status}")
        ...
        self.delete_button.setEnabled(has_tasks)
        self.duplicate_button.setEnabled(has_tasks)

    def _request_duplicate(self) -> None:
        self.duplicate_task_requested.emit(self.list.currentRow())
